...
int main(int argc, char **argv){
  ...
  if(argc < 2){                 // Set TIME_OF_DAY_SEC to the actual time
    ...
    CLOCK_TIME_PORT = (info->tm_sec + 60*info->tm_min + 60*60*info->tm_hour)*16;
  }
  else{                         // Set time based on argument given
    CLOCK_TIME_PORT = atoi(argv[1]);
  }
  printf("CLOCK_TIME_PORT set to: %d\n", CLOCK_TIME_PORT);
  ...
  printf("result: %d\n",result);
  if(result != 0){
    printf("failed to initialize tod; bailing\n");
    return 1;
  }
  printf("tod = {\n"); 
  printf("  .day_secs   = %d\n",tod.day_secs);
