...
void print_ports(){
  printf("%-18s : %d\n","CLOCK_TIME_PORT", CLOCK_TIME_PORT);
...
int CALL_set_tod_from_ports(tod_t *);
int CALL_set_display_from_tod(tod_t, int *);
int CALL_clock_update();
...
int main(int argc, char *argv[]){
  ...
  int *dispint = malloc(sizeof(int)); // heap allocate to allow valgrind to check
  tod_t *tod = malloc(sizeof(tod_t)); // memory accesses / detect uninitialized
  int ret = -1;
  if(0){}
  ...
  if( all==1 || strcmp( test_name, "field-ranges-display1" )==0 ) {
    PRINT_TEST;
  ...
  if( all==1 || strcmp( test_name, "field-ranges-display2" )==0 ) {
    PRINT_TEST;
    // Tests whether set_display_from_tod() checks for
    // ranges of hours, minutes, seconds, etc. and errors
    // out if they are invalid.
  ...
  if( all==1 || strcmp( test_name, "bad-port-range-set1" )==0 ) {
    PRINT_TEST;
  ...
  if( all==1 || strcmp( test_name, "bad-port-range-set2" )==0 ) {
    PRINT_TEST;
  ...
  if( all==1 || strcmp( test_name, "bad-port-range-update1" )==0 ) {
    PRINT_TEST;
  ...
  if( all==1 || strcmp( test_name, "bad-port-range-update2" )==0 ) {
    PRINT_TEST;
